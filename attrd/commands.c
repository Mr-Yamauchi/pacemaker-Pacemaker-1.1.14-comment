 ...
 * \return void
 */
/* attrのIPCクライアント(attrd_updterなど)からの更新処理 */
void
attrd_client_update(xmlNode *xml)
{
    ...
    if ((peer_writer == NULL) && (election_state(writer) != election_in_progress)) {
		/* クライアントからの属性更新を受信した時に、まだ、ELECTIONが開始されていない場合は、 */
		/* election_vote()でELECTIONを開始する */
    ...
    free(set);
    free(host);
	/* クライアントからの更新処理メッセージをattrd間で送信する */
    send_attrd_message(NULL, xml); /* ends up at attrd_peer_message() */
}
...
attrd_peer_message(crm_node_t *peer, xmlNode *xml)
{
    ...
    if(election_op) {
        ...
        switch(rc) {
            case election_start:
                ...
                break;
            case election_lost:
            	/* 自ノードがelection_lost(非OWNER)の場合は、メッセージ送信元をpeer_writerにセットする */
            	/* この時、peer_writerはelection_won(OWNER) */
                free(peer_writer);
    ...
    if(election_state(writer) == election_won
       ...
       && safe_str_neq(peer->uname, attrd_cluster->uname)) {
        ...
        /* 自ノードがelection_wonで、他ノードもelection_wonの場合は、再度ELECTIONを開始する為に */
        /* election_vote()を実行する */
        election_vote(writer);

    } else if(peer_state == election_won) {	/* 送信元がelection_won(OWNER)だった場合 */
        if(peer_writer == NULL) {
			/* 自ノードのpeer_writer情報が空の場合は、送信元のノード名をpeer_writerにセットする */
            peer_writer = strdup(peer->uname);
            crm_notice("Recorded attribute writer: %s", peer->uname);

        } else if(safe_str_neq(peer->uname, peer_writer)) { /* 元のpeer_writerと異なるノードが、election_won(OWNER)になった場合 */
            crm_notice("Recorded new attribute writer: %s (was %s)", peer->uname, peer_writer);
            /* 一旦、peer_writerを開放して、新しいelection_won(OWNER)の送信元のノード名をpeer_writerにセットする */
            free(peer_writer);
            peer_writer = strdup(peer->uname);
        }
    }
...
attrd_peer_update(crm_node_t *peer, xmlNode *xml, const char *host, bool filter)
{
    ...
    if(changed) {
        ...
        if(a->timer) {
            ...
            /* 属性(delay--dampen)タイマースタート */
            mainloop_timer_start(a->timer);
            /* タイマーコールバック内で、write_or_election_attribute()を実行しているので、 */
            /* ELECTIONがelecion_won(OWNER)のattrdは属性を実際に更新する */
            /* election_wonでないノードからは属性は更新されない */
        } else {
            /* ELECTIONがelecion_won(OWNER)のattrdは属性を実際に更新する */
            /* election_wonでないノードからは属性は更新されない */
            write_or_elect_attribute(a);
...
/* 属性書き込み処理 */
/* 実際に書き込みを実施するのは、election_won(OWNER)のノードのattrdのみで、その他は書き込みはしない */
void
write_or_elect_attribute(attribute_t *a)
{
    ...
    if(rc == election_won) {
		/* 自ノードがelection_won(OWNER)の場合は、属性を書き込む */
        write_attribute(a);
        ...
        /* peer_writerが未設定の場合は、ELECTIONを開始する為にelection_vote()を実行する */
        election_vote(writer);
    ...
    if ((kind == crm_status_nstate) || (kind == crm_status_rstate)) {
        if (safe_str_eq(peer->state, CRM_NODE_MEMBER)) {
            ...
            if (peer_writer && safe_str_eq(peer->uname, peer_writer)) {
				/* 構成が変更されて、離脱したノードがpeer_writeだった場合は、peer_writerをクリアする */
                free(peer_writer);
                peer_writer = NULL;
